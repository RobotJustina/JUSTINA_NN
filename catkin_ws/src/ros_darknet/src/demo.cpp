...
//#ifdef DARKNET_DIR
std::string darknetFilePath_ = "/home/biorobotica/darknet";
...
void *YoloObjectDetector::publishInThread()
{
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          /*/////////////////////////////7boundingBox.Class = classLabels_[i];
    ...
    /*/////////////////////////////////7boundingBoxesResults_.header.stamp = ros::Time::now();
  ...
  if (isCheckingForObjects()) {
    ...
    //////////////////77objectsActionResult.id = buffId_[0];
    /////////////////objectsActionResult.bounding_boxes = boundingBoxesResults_;
    ////////////////7checkForObjectsActionServer_->setSucceeded(objectsActionResult, "Send bounding boxes.");
  }
